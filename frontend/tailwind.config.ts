...
/**
 * 墨卷 · Ink & Vermilion 设计系统令牌。
 *
 * 语义色经 CSS 变量（globals.css）在亮/暗主题间切换，构建期映射为 Tailwind 工具类，
 * 支持 <alpha-value> 透明度修饰（如 bg-accent/15）。
 * 命名约定：canvas=画布底、surface=卡片面、ink=墨色文字、line=描边、accent=朱砂主色。
 */
const config: Config = {
  darkMode: "class",
  content: ["./src/**/*.{ts,tsx}"],
  theme: {
    extend: {
      ...
      fontFamily: {
        // 中文字体优先本机字体栈，离线可用；sans=界面正文，serif=卷宗标题，kai=批注/印章
