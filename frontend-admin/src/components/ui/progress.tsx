...
const FILLS: Record<Tone, string> = { accent: "bg-accent", ok: "bg-ok", warn: "bg-warn", danger: "bg-danger", info: "bg-info" };
...
export function Progress({ value, tone = "accent", className = "" }: { value: number; tone?: Tone; className?: string }) {
  ...
  return (
    <div role="progressbar" aria-valuenow={Math.round(v)} aria-valuemin={0} aria-valuemax={100}
      ...
      <div className={`h-full rounded-full transition-[width] duration-300 ease-out ${FILLS[tone]}`} style={{ width: `${v}%` }} />
    </div>
  );
}