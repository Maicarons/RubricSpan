...
| 项 | 值 |
|---|---|
| GPU | NVIDIA GPU（≥8GB 显存，Ada Lovelace 或更新架构） |

## 软件栈

| 组件 | 版本 | 说明 |
|---|---|---|
| Python | 3.14 | 系统 Python（Windows） |
| PyTorch | 2.11.0+cu128 | CUDA 12 官方轮子，`torch.cuda.is_available()=True` |
...
3. **CUDA EP 的 DLL 解析**：ORT 找不到 cuDNN 时，把 PyTorch 自带的运行库目录
   （site-packages 下的 `torch/lib`，含 cublas64_12 / cublasLt64_12 / cudnn64_9 等）
